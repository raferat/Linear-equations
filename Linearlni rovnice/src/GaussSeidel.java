...
public class GaussSeidel extends IteratingCalculation 
{
	double [] prediction;
	double [] lastPrediction;
	
	public GaussSeidel( double [] prediction) 
	{
		super ();
		this . prediction = prediction;
	}
	
	public GaussSeidel( double [] prediction , int iterations) 
	{
		super (iterations);
		this . prediction = prediction;
	}

	public GaussSeidel( double [] prediction , double exactness) 
	{
		super (exactness);
		this . prediction = prediction;
	}

	public GaussSeidel( double [] prediction , double exactness , int iterations) 
	{
		...
		do
		{
		  ...
		  for (int y = 0 ; y < e.getKoef().width ; y ++ )
		  {
			  ...
			  for ( int x = 0 ; x < e.getKoef().height ; x ++ )
			  {
				  if ( x != y )
				  {
				     citatel -= e.getKoef().get(y, x) * prediction[x];
				  }
			  }
